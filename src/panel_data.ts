// Panel verisi: state/ dosyalarını okur, tek JSON üretir. Hem canlı panel (dashboard.ts) hem statik anlık görüntü (scripts/export_panel.ts) bunu kullanır.
import { existsSync, readFileSync } from "node:fs";

export const read = (p: string) => (existsSync(p) ? readFileSync(p, "utf-8") : "");
export const json = (p: string, d: any) => { try { return JSON.parse(read(p) || "null") ?? d; } catch { return d; } };
export const jsonl = (p: string) => read(p).trim().split("\n").filter(Boolean).map((l) => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
