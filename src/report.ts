...
const snaps = lines.filter((l) => l.kind === "snapshot").map((l) => ({ ts: l.ts, eq: Number((l.msg.match(/(?:özkaynak|kasa) ([\d.]+)/) ?? [])[1]) })).filter((s) => s.eq > 0);
...
| Metrik | Değer |
|---|---|
| Başlangıç kasa | ${start.toFixed(2)} USDT |
| Bitiş kasa | ${end.toFixed(2)} USDT |
...
| Filtre kapalı sayısı | ${count("gate")} |
| Hata | ${count("error")} |
